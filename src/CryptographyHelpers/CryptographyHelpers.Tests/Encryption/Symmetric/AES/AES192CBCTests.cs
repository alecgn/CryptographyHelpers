...
namespace CryptographyHelpers.Tests.Encryption.Symmetric.AES
{
    ...
    public class AES192CBCTests
    {
        ...
        {
            var valid192BitsKey = CryptographyUtils.GenerateRandom192BitsKey();
            var validEncoded192BitsKey = encodingType switch
            {
                EncodingType.Base64 => _base64Encoder.EncodeToString(valid192BitsKey),
                EncodingType.Hexadecimal => _hexadecimalEncoder.EncodeToString(valid192BitsKey),
                _ => throw new InvalidOperationException($@"Unexpected enum value ""{encodingType}"" of type {typeof(EncodingType)}."),
            };

            Action act = () => { using var aesGcm = new AES192CBC(validEncoded192BitsKey, invalidEncodedIV, encodingType); };
