...
namespace CryptographyHelpers.Tests.Encryption.Symmetric.AES
{
    ...
    public class AES256CBCTests
    {
        ...
        {
            var valid256BitsKey = CryptographyUtils.GenerateRandom128BitsKey();
            var validEncoded256BitsKey = encodingType switch
            {
                EncodingType.Base64 => _base64Encoder.EncodeToString(valid256BitsKey),
                EncodingType.Hexadecimal => _hexadecimalEncoder.EncodeToString(valid256BitsKey),
                _ => throw new InvalidOperationException($@"Unexpected enum value ""{encodingType}"" of type {typeof(EncodingType)}."),
            };

            Action act = () => { using var aesGcm = new AES128CBC(validEncoded256BitsKey, invalidEncodedIV, encodingType); };
